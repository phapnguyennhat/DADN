...
export default function CurrentLedStatus({ modeLed }: IProps) {
    ...
    useEffect(() => { 
        ...
        if (+lightData > 50 && ledStatus === '1' && modeLed === 'auto') {
            createLogLight.mutate({ lightIntensity: +lightData, typeLog: 'auto', lightStatus: 'on' })
            handleSetLedStatus('0')
        } else if (+lightData <= 50 && ledStatus === '0' && modeLed === 'auto') {
            createLogLight.mutate({ lightIntensity: +lightData, typeLog: 'auto', lightStatus: 'off' })
            handleSetLedStatus('1')
        }
    }, [lightData, ledStatus, modeLed])
    
    const disabled = useMemo(()=> (createLogLight.isPending || modeLed === 'auto'), [modeLed, createLogLight.isPending])

    
    return (
        <View className="w-[390px]  px-5 mx-auto h-[228px] bg-[#10493F] rounded-[24px] flex-row items-center gap-[10px] justify-center  ">
           {ledStatus === '1' ? (
            <TouchableOpacity
                disabled={disabled}
                onPress={()=>{
                    handleSetLedStatus('0')
                    createLogLight.mutate({lightIntensity: +lightData, typeLog: 'manual', lightStatus: 'off'})
                }}
            ...
            <TouchableOpacity
                ...
                onPress={()=>{
                    handleSetLedStatus('1')
                    createLogLight.mutate({lightIntensity: +lightData, typeLog: 'manual', lightStatus: 'on'})
                }}
